	...
	(function () {
			...
			GM_xmlhttpRequest({
				...
				onload: function (response) {
					...
					$('.posttitle .userinfo .span3 .user', holder).each((j, d, n) => {
						...
						var src = $(d.parentNode.parentNode.parentNode.parentNode.parentNode).find('img').attr('src');
		...
		var compareName = (nameA, nameB, tag) => {
			const name1 = nameA.replace(`【${tag}】`, '').replace(`[${tag}]`, '').replace(`${tag}`, '').toLowerCase().trim();
			const name2 = nameB.replace(`【${tag}】`, '').replace(`[${tag}]`, '').replace(`${tag}`, '').toLowerCase().trim();
